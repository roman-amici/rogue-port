...
pub fn player_input(mut query: Query<(&Player, &mut WorldPosition)>, input_manager : ResMut<InputManager>, map : Res<Map>, mut camera : ResMut<Camera>){
    let delta = input_manager.keys
    .iter()
    ...
    for (_,mut pos) in query.iter_mut() {
        ...
        if map.player_can_enter(destination) {
