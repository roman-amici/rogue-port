...
fn min_max_distance(center: Point, point: Point) -> (Point, Point) {
    ...
    let mut min_distance = cartesian_distance(center, point);
    ...
    for p in [px1y, pxy1, px1y1] {
        let d = cartesian_distance(center, p);
        if d < min_distance {
            min_distance = d;
            min_point = p;
        }
...
fn circle_points(position: Point, radius: i32) -> HashSet<Point> {
    ...
    for x in min_x..=max_x {
        for y in min_y..=max_y {
            ...
            let min_distance = cartesian_distance(min_point, position);
            let max_distance = cartesian_distance(max_point, position);
