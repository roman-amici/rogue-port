...
impl HudRender {
    ...
    fn render_tooltip(
        ...
        let tile_start =
            TileRender::tile_to_screen_space(self.tile_screen_size, tile_point.y, tile_point.x);
        ...
        canvas
            .fill_rect(text_box)
            .expect("failed to rsprite_indexender rect.");
    ...
    fn render_healthbar(
        ...
        canvas
            .fill_rect(Rect::new(0, 0, screen_width, self.health_bar_height as u32))
            .expect("Failed to draw healthbar");

        let colored_width = (screen_width as f32 * fraction) as u32;
        canvas.set_draw_color((255, 0, 0));
        canvas
            .fill_rect(Rect::new(0, 0, colored_width, text_box.height()))
            .expect("Failed to draw healthbar");
    ...
    fn render_inventory(
        ...
        for (index, item) in items.iter().enumerate() {
            ...
            let text_size = Rect::new(
                point.x,
                point.y,
                self.tile_screen_size / 4,
                self.tile_screen_size / 4,
            );
            text_render.render_text_in_cache(&format!("{}", index), text_size, canvas);
